# Decode Labs — Data Analyst Internship

Welcome to my Data Analyst Internship repository at 
**Decode Lab**! This repository contains all 4 projects 
completed during my internship.

---

## About
- **Company:** Decode Lab
- **Role:** Data Analyst Intern
- **Duration:** 28 May 2026 - 28 June 2026
- **Tools:** Python, Pandas, NumPy, 
  Matplotlib, Jupyter Notebook, Excel

---

## Projects Overview
| # | Project | Description | Status |
|---|---------|-------------|--------|
| 1 | Data Cleaning & Preparation | Cleaned 1200-row e-commerce dataset | Done |
| 2 | Exploratory Data Analysis(EDA) | Analyze Patterns, Trends & Distribution | Done |
| 3 | Coming Soon | - | Pending |
| 4 | Coming Soon | - | Pending |

---

## Tools & Technologies
- Python 3
- Pandas & NumPy
- Jupyter Notebook
- Microsoft Excel
- GitHub

---

## Skills Gained
- Data Cleaning & Preparation
- Exploratory Data Analysis
- Data Visualization
- Business Insights & Reporting

---

## Author
**Shivika** — Data Analyst Intern @ Decode Lab  
